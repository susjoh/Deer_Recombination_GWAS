...
#~~ Read in phenotype file and provide data for paper
...
rectab.raw <- read.table(paste0("results/2_Per_Chromosome_Recomb_raw_", AnalysisSuffix, ".txt"), header = T, stringsAsFactors = F)

head(rectab.raw)

rectab.raw <- subset(rectab.raw, select = c(data, RecombCount, parent, Family, RRID, analysisID, UniqueID))
rectab.final <- subset(rectab, select = c(data, RecombCount, UniqueID))
names(rectab.final) <- c("data.QCed.if.different", "RecombCount.QCed", "UniqueID")

rectab.raw <- join(rectab.raw, rectab.final)
table(rectab.raw$RecombCount == rectab.raw$RecombCount.QCed)
table(rectab.raw$data == rectab.raw$data.QCed.if.different)

rectab.raw$data.QCed.if.different[which(rectab.raw$data == rectab.raw$data.QCed.if.different)] <- NA
head(rectab.raw)

rectab.raw <- subset(rectab.raw, select = c(RRID, Family, parent, analysisID, RecombCount, RecombCount.QCed, data, data.QCed.if.different))
names(rectab.raw) <- c("FocalID", "OffspringID", "Parent", "Chromosome", "ACC.raw", "ACC.QCed", "Phase.raw", "Phase.QCed.If.Different")
rectab.raw$Chromosome <- gsub("a", "", rectab.raw$Chromosome)
rectab.raw$OffspringID <- sapply(rectab.raw$OffspringID, function(x) strsplit(x, split = "_")[[1]][2])

write.table(rectab.raw, "results/4_Crossover_Positions_Counts_Raw_QCed.txt", row.names = F, sep = "\t", quote = F)
#~~ 
...
#~~ reviewer comment about dominance variance

# Calculating the dominance GRM from all the autosomal SNPs
system("gcta\\gcta.exe --bfile gcta\\Deer31.recoded.v2 --autosome --make-grm-d --out gcta\\Deer31.recoded.v2.grm.dom")
system("gcta\\gcta.exe --bfile gcta\\Deer31.recoded.v2 --autosome --make-grm-d --out gcta\\Deer31.recoded.v2.grm.dom")

grm.auto.dom <- read.table("gcta/Deer31.recoded.v2.grm.dom.grm.gz")  # CONTAINS REALIZED RELATEDNESS BETWEEN ALL GENOTYPED INDIVIDUALS
ids.auto.dom <- read.table("gcta/Deer31.recoded.v2.grm.dom.grm.id")  # CONTAINS ID LIST

grminv.dom <- makeGRM(grm.auto.dom, ids.auto.dom, recsumm$RRID) # vector of IDs from the datasset that you use for the asreml model
dim(grminv.dom)
...
grm.summ.RR <- asreml(fixed = TotalRecombCount ~ RRID.Sex + RRID.Fhat3,
                      ...
                      data = recsumm,
                      ginverse =  list(RRID = grminv, RRID2 = grminv.dom),
                      na.method.X = "omit", na.omit.Y = "na.omit",
                      workspace = 500e+6, pworkspace = 500e+6)
