...
rectab <- read.table("results/2_Per_Chromosome_Recomb_Final_a.txt", header = T, sep = "\t")
head(rectab)

rectab <- subset(rectab, CEL.LG == 12)
rectab <- subset(rectab, select = c(RRID, ANIMAL, parent, Family, RecombCount, UniqueID))

head(recsumm)

recsumm <- plyr::join(recsumm, rectab)
recsumm.m <- plyr::join(recsumm.m, rectab)
recsumm.f <- plyr::join(recsumm.f, rectab)

recsumm$TotalRecombCount <- recsumm$TotalRecombCount - recsumm$RecombCount
recsumm.m$TotalRecombCount <- recsumm.m$TotalRecombCount - recsumm.m$RecombCount
recsumm.f$TotalRecombCount <- recsumm.f$TotalRecombCount - recsumm.f$RecombCount
...
for(i in regh2.tab$Analysis.ID[1:nrow(regh2.tab)]){
  ...
  try({
    ...
    grm.rest <- read.table(paste0("gcta/regh2_", AnalysisSuffix, "/", i,"_wo_GRM_adj.grm.gz"))
    ids.rest <- read.table(paste0("gcta/regh2_", AnalysisSuffix, "/",i,"_wo_GRM_adj.grm.id"))
    ...
    save(x, file = paste0("gcta/regh2_", AnalysisSuffix2, "/", i,"_trans.RData"))
